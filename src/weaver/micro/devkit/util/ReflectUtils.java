...
public final class ReflectUtils {
    ...
    public static Class<?>[] getAllSuper(Class<?> clazz) {
        ...
        while (clazz != null) {
            ...
            if ((needSize = count + interfaces.length) >= classes.length) {
                if (clazz == null)
                    classes = ArrayUtils.arrayExtend(classes, needSize);
                else
                    classes = ArrayUtils.arrayExtend(classes, needSize + 4);
            }
        ...
        if (count == classes.length)
            return ArrayUtils.delRepeat(classes);
        else {
            return ArrayUtils.delRepeat(ArrayUtils.arrayExtend(classes, count));
        }
    }
    ...
    public static Field[] queryFields(Class<?> clazz, final int filter, boolean parent) {
        ...
        if (parent) {
            clazz = clazz.getSuperclass();
            while (clazz != null) {
                fields = ArrayUtils.concat(fields, clazz.getDeclaredFields());

                clazz = clazz.getSuperclass();
            }
        }

        fields = ArrayUtils.arrayFilter(fields, new ArrayUtils.ArrayFilter<Field>() {
            @Override
